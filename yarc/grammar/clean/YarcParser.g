...
settings:
	SETTINGS COLON NEWLINE INDENT (setting | code_snippet)+ DEDENT;
setting: ID ASSIGN test NEWLINE;

writers:
	WRITERS COLON NEWLINE INDENT (
		expr_stmt
		| code_snippet
		| writer
	)+ DEDENT;
...
instantiate_expr:
	INSTANTIATE test? FROM test (edit_block | NEWLINE);
group_expr:
	GROUP LBRACK test (COMMA test)* RBRACK (edit_block | NEWLINE);
get_expr:
	GET ((CAMERA | LIGHT | MATERIAL | ID) AT)? test (
		simple_block
		| NEWLINE
	);

edit_block:
	COLON NEWLINE INDENT (attr | inner_behavior_stmt)+ DEDENT;
...
testlist_comp:
	test (comp_for | (COMMA test)* | COLON test (COLON test)?);
